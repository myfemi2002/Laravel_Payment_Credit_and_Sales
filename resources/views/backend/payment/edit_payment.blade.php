                    ...
                    <table id="example" class="cell-border display table" style="width:100%">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    <button class="btn btn-sm btn-primary edit-data-btn" title="Edit Data" data-bs-toggle="modal" data-bs-target="#editModal" data-item="{{ json_encode($item) }}">Edit Payment</button>
                                </td>
                            </tr>
                            @endforeach
                        </tbody>
            ...
            <div class="modal-body p-4">
                <form class="needs-validation" id="myForm" method="POST" action="{{ route('unpaid-payment.update') }}" enctype="multipart/form-data" novalidate>
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <input class="form-control" id="customer_name" placeholder="Customer Name" required>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="mb-3">
                                <label for="purchase_date" class="form-label">Purchase Date</label>
                                <input class="form-control" id="purchase_date" placeholder="Purchase Date" required>
                            </div>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" id="product_total_amount" placeholder="Product Total Amount" required>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="mb-3">
                                <label for="amount_paid" class="form-label">Amount Paid</label>
                                <input type="text" class="form-control" id="amount_paid" placeholder="Amount Paid" name="amount_paid" oninput="calculateTotalAmountPaid(); updatePaymentStatus(); validateAmountPaid()" required>
                            </div>
                        </div>
                    </div>
                    <div class="row">
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-control form-small select" name="event_name" required>
                                    ...
                                    @foreach($event as $item)
                                        <option value="{{ $item->event_name }}" {{ $item->event_name === old('event_name', $payment->event_name ?? '') ? 'selected' : '' }}>
                                            {{ $item->event_name }}
                                        </option>
                                    @endforeach
                                </select>
                    ...
                    <div class="modal-footer mt-3">
                        ...
                        <button type="submit" class="btn btn-info waves-effect waves-light" onclick="return validateForm()">Save changes</button>
                    </div>
                </form>
            </div>
        </div>
...
<script>
    ...
    editButtons.forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            paymentIdField.value = itemData.id;
            ...
            eventNameField.value = itemData.event_name;
        });
    });
